...
local function Func_AutoCrystalDefense()
    while Toggles.AutoCrystalDefense.Value do
        ...
        local isInCrystal = npcsFolder and game.PlaceId == 98826438856089, 130167267952199
        if isInCrystal then
...
local Window = Library:CreateWindow({
	Title = "NixHub",
	Footer = "" .. assetName .. " | Version 1.0",
	NotifySide = "Right",
    Icon = "rbxasset://91636465138754",
	ShowCustomCursor = true,
	AutoShow = true,

-- [[ FLUENT UI SETUP & BRIDGE ]]
...
-- We create the Toggles and Options tables that the original script expects
local Toggles = {}
local Options = {}

-- Example of mapping (You can add more as needed, but the logic will now find these tables)
...
-- Infinite Tower Modules Integration (As requested)
if Modules then
    Modules.InfiniteTowerMerchantConfig = GetSafeModule(RS.Modules, "InfiniteTowerMerchantConfig")
    Modules.InfiniteTower = GetSafeModule(RS.Modules, "InfiniteTowerConfig")
    Modules.InfiniteTowerData = GetSafeModule(RS.Modules, "InfiniteTowerData")
    Modules.InfiniteTowerRewards = GetSafeModule(RS.Modules, "InfiniteTowerRewards")
end
    Callback = function(a)
        tonumber(a)
    end
...
TB_Tabs.Dungeon.T1:AddToggle("AutoCrystalDefense", {
    ...
    Default = false,
    Tooltip = "Supports Crystal Defense (Place ID: 98826438856089, 130167267952199)"
})
